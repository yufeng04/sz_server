...
function api(action, req, res) {
    ...
    if (mongo.isConnected()) {
        ...
        mongo.connect(CONFIG.mongodburl).then(function (result) {
            if (result) {
...
function getData(req, res) {
    ...
    var filterType = http.getRequest(req, "filterType");
    var module = http.getRequest(req, "module");
    var status = http.getRequest(req, "status");
                ...
                result.forEach(function (ticket) {
                    ...
                    Promise.all([fetchData(teamid, module, filterCondition, "or"), fetchMetaData(teamid, module)]).then(function (resultList) {
                        if (resultList instanceof Array && resultList.length > 1) {
                            var customerData = resultList[0];
                            var metaResult = resultList[1];
                            var clientInforList = [];
                            if (customerData.result == 'SUCCESS' && metaResult.result == 'SUCCESS') {
                                var customerList = customerData.list;
                                var metaList = metaResult.list;
...
function addData(req, res) {
    ...
    if (typeof teamid == 'undefined' || teamid === null || typeof ownername == 'undefined' || ownername === null) {
        res.json({
            result: "FAIL",
            errMsg: "invalid inputs"
        });
    ...
    Promise.all([getUserInfor(teamid, ownername), getUserInfor(teamid, worker)]).then(function (result) {
        if (result instanceof Array && result.length > 1) {
            ...
            var ticketInfor = {
                ...
                dept: dept,
                worker: {
                    name: _worker ? _worker.nickname : "",
                    phone: _worker ? _worker.phonenumber : "",
                    email: _worker ? _worker.email : ""
                },
                type: ticketType,
...
function commentData(req, res) {
    ...
    var worker = http.getRequest(req, "worker");
    var action = http.getRequest(req, "action");
...
function fetchData(teamid, module, filters, filter_operator) {
    ...
    if (filters == '' || filters == null) filter_operator = 'or';
